...
interface WorkflowConfig {
  ...
  labels: {
    inProgress: string;       // default: "status:in-progress"
    inReview: string;         // default: "status:in-review"
  };
...
- Issue ラベルを `status:in-progress` に更新
...
3. `IIssueGateway.updateLabels()` で `status:in-review` に更新

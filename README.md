...
`ai-scrum task`（引数なし）を実行すると、`ai-scrum:issue:status:ready` ラベルが付いた Task Issue の中から優先度が最も高い 1 件を自動選択し、タスク実装ワークフローを起動します。

```
Ready な Task Issue を優先度順にソート → 最優先の 1 件を選択 → ワークフロー 3 を実行
```

優先度は以下のラベルで判別します（上が高優先）：

| ラベル | 優先度 |
|-------|--------|
| `ai-scrum:issue:priority:P0-immediately` | 最優先（即時対応） |
| `ai-scrum:issue:priority:P1-high` | 高 |
| `ai-scrum:issue:priority:P2-medium` | 中 |
| `ai-scrum:issue:priority:P3-low` | 低 |
| （ラベルなし） | 最低 |

同じ優先度の Issue が複数ある場合は、Issue 番号の小さいもの（＝作成が古いもの）を優先します。
...
| `ai-scrum:issue:priority:P0-immediately` | 優先度: 最優先（即時対応） |
| `ai-scrum:issue:priority:P1-high` | 優先度: 高 |
| `ai-scrum:issue:priority:P2-medium` | 優先度: 中 |
| `ai-scrum:issue:priority:P3-low` | 優先度: 低 |
...
# 引数なし → ready な Issue から最優先の 1 件を自動選択して実行
npx ai-scrum task

# どの Issue が選ばれるか確認だけ（実行しない）
npx ai-scrum task --dry-run
